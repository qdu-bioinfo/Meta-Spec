...
def get_msi(args):
    ...
    target = args.disease_name.split(" ")  
    
    sparse_features = []
    if len(args.host) > 0:
        sparse_features = [x for x in df_hosts.columns if x!="SampleID"]

    train_model = torch.load(args.msi_model)
    
    pkl_file = open(args.o+'dnn_input_train.pickle', 'rb')
    dnn_input_train = pickle.load(pkl_file)
    pkl_file = open(args.o+'dnn_input_test.pickle', 'rb')
    ...
    for i in range(len(target)):
        ...
        sta.to_csv(args.o + target[i]+'_msi.csv',index=False)               
    ...
    if args.is_plot:
        for i in range(len(target)):
            ...
            pl.savefig(args.o+target[i]+'_top'+str(args.max_plot)+'.jpg', dpi=300, bbox_inches = 'tight')
